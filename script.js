...
function iniciar() {
    ...
    if (acao.value == 0) {
        document.getElementById('erro_acao').innerHTML = "Adcione os minutos"
        acao.focus()
    } else if (pausa.value == 0) {
        document.getElementById('erro_pausa').innerHTML = "Adcione a pausa"
        pausa.focus()
    } else if (sessoes.value == 0) {
        document.getElementById('erro_sessoes').innerHTML = "Adcione as sessões"
        sessoes.focus()
    } else {
        lofi.play()
        pause.style.setProperty('display', 'block', 'important')
